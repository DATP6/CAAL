...
QUnit.test('Simple not bisimilar', function (assert) {
    ...
    assert.ok(
        !Equivalence.isBisimilar(strongSuccGen, strongSuccGen, processP.id, processQ.id, graph),
        'P and Q should not be bisimilar'
    );
});
...
QUnit.test('Weak bisimilar', function (assert) {
    ...
    assert.ok(
        !Equivalence.isBisimilar(strongSuccGen, weakSuccGen, processP.id, processQ.id, graph),
        'S and T are not weakly bisimilar'
    );
});
...
QUnit.test('Weak bisimilar 2', function (assert) {
    ...
    assert.ok(
        Equivalence.isBisimilar(strongSuccGen, weakSuccGen, processP.id, processQ.id, graph),
        'S and T should be weakly bisimilar'
    );
});

QUnit.test('Tau Cycles', function (assert) {
    var graph = CCSParser.parse('P = a.tau.b.P2;\n' + 'P2 = tau.P2;\n' + 'Q = a.tau.b.tau.tau.0;\n', { ccs: ccs }),
        strongSuccGen = getStrictSuccGenerator(graph),
        weakSuccGen = getWeakSuccGenerator(graph),
        processP = graph.processByName('P'),
        processQ = graph.processByName('Q');
...
QUnit.test('Alternative Bit Protocol', function (assert) {
    ...
    assert.ok(
        Equivalence.isBisimilar(attackSuccGen, defendSuccGen, protocol, spec, graph),
        'ABP Protocol should be bisimilar with Spec'
    );
});
...
QUnit.test('Alternative Bit Protocol', function (assert) {
    ...
    assert.ok(
        Equivalence.isBisimilar(attackSuccGen, defendSuccGen, protocol, spec, graph),
        'Modified ABP Protocol should be bisimilar with Spec'
    );
});
...
QUnit.test('Bad CM and CS', function (assert) {
    ...
    assert.ok(
        !Equivalence.isBisimilar(attackSuccGen, defendSuccGen, protocol, spec, graph),
        'System and Spec is not be bisimilar'
    );
});
