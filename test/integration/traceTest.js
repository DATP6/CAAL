...
QUnit.test('Simple strong trace equivalence', function (assert) {
    ...
    assert.ok(
        Equivalence.isTraceIncluded(strongSuccGen, strongSuccGen, processP.id, processQ.id, graph).isSatisfied,
        'P should be trace included in Q'
    );
    assert.ok(
        Equivalence.isTraceIncluded(strongSuccGen, strongSuccGen, processQ.id, processP.id, graph).isSatisfied,
        'Q should be trace included in P'
    );
});
...
QUnit.test('Simple one way inclusion', function (assert) {
    ...
    assert.ok(
        !Equivalence.isTraceIncluded(strongSuccGen, strongSuccGen, processQ.id, processP.id, graph).isSatisfied,
        'Q should not be trace included in P'
    );
});
...
QUnit.test('Simple weak trace inclusion', function (assert) {
    ...
    assert.ok(
        !Equivalence.isTraceIncluded(strongSuccGen, strongSuccGen, processP.id, processQ.id, graph).isSatisfied,
        'P should not be strongly trace included in Q'
    );
    assert.ok(
        Equivalence.isTraceIncluded(weakSuccGen, weakSuccGen, processP.id, processQ.id, graph).isSatisfied,
        'P should be weakly trace included in Q'
    );
});
...
QUnit.test('Peterson', function (assert) {
    ...
    var isTraceIncluded = Equivalence.isTraceIncluded(
        attackSuccGen,
        defendSuccGen,
        leftProcess.id,
        rightProcess.id,
