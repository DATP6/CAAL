...
QUnit.test('Others (simple)', function (assert) {
    assert.ok(checkFormula('P = a.Q + a.R + y.P;' + 'Q = b.P;' + 'R = b.P;', 'P', '[a]<b>[z]ff;'), 'should be true');

    assert.ok(
        checkFormulaForVariable(
...
QUnit.test('Distinguishing Formula Greedy Simplication', function (assert) {
    var program = ['P = a.(c.0 + d.0);', 'Q = a.b.0 + a.c.0;'].join('\n'),
        leftProcess = 'P',
        rightProcess = 'Q';
    ...
    assert.ok(
        formula.id === formulaSet.getTopLevelFormulas()[0].id,
        'Simplified correctly (<a>(<c>T and <d>T);) to <a><d>T'
    );
