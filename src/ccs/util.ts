...
module Traverse {
        ...
        dispatchRestrictionProcess(process: ccs.RestrictionProcess) {
            ...
            if (!result) {
                ...
                subStr = wrapIfInstanceOf(subStr, process.subProcess, [
                    ccs.SummationProcess,
                    ccs.CompositionProcess,
                    ccs.ActionPrefixProcess
                ]);
            ...
            if (!result) {
                result =
                    ...
                    process.subProcesses
                        .map((subProc) => (subProc instanceof ccs.NamedProcess ? subProc.name : subProc.id))
                        .join(', ') +
                    '}';
            }
            return result;
    ...
    export class HMLNotationVisitor implements hml.FormulaVisitor<string>, hml.FormulaDispatchHandler<string> {
        ...
        constructor(
            private showSemicolon = true,
            private formatComplement = true,
            private safeHtml = true
        ) {
            this.clearCache();
        }
    ...
    export class HMLSuccGenVisitor
        implements HML.FormulaVisitor<Array<HML.Formula>>, HML.FormulaDispatchHandler<Array<HML.Formula>>
    {
        ...
        dispatchVariableFormula(formula: HML.VariableFormula) {
            var result = [];
            var namedFormulaDef = <HML.MinFixedPointFormula | HML.MaxFixedPointFormula>(
                this.hmlFormulaSet.formulaByName(formula.variable)
            );
    ...
    export class HMLSimplifier implements hml.FormulaDispatchHandler<hml.Formula> {
        ...
        dispatchWeakExistsFormula(formula: hml.WeakExistsFormula) {
            ...
            if (subFormula instanceof hml.WeakExistsFormula) {
                if (
                    formula.actionMatcher instanceof hml.SingleActionMatcher &&
                    formula.actionMatcher.matches(new CCS.Action('tau', false))
                ) {
                    //  <<tau>> <<X>> ...
                    return subFormula;
                } else if (
                    subFormula.actionMatcher instanceof hml.SingleActionMatcher &&
