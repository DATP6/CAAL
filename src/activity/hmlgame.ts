...
module Activity {
    ...
    class HmlGameActivity {
        ...
        */
        ...
        constructor(container: string) {
            ...
            this.fullscreen = new Fullscreen($('#hml-game-container')[0], $('#hml-game-fullscreen'), () =>
                this.resize()
            );
        ...
        onShow(configuration) {
            ...
            if (
                ...
                configIsSame(this.configuration, configuration) ||
        ...
        private configure(configuration): void {
            ...
            this.strongSuccGen = CCS.getSuccGenerator(this.graph, {
                inputMode: InputMode[this.project.getInputMode()],
                succGen: 'strong',
                reduce: true
            });
            this.weakSuccGen = CCS.getSuccGenerator(this.graph, {
                inputMode: InputMode[this.project.getInputMode()],
                succGen: 'weak',
                reduce: true
            });
            ...
            this.hmlGameLogic = new HmlGameLogic(
                currentProcess,
                currentFormula,
                currentFormulaSet,
                this.strongSuccGen,
            ...
            gameIntro.setTemplate(
                'You are playing {0} in HML game.</br> {1} has a winning strategy. You are going to lose.'
            );
            gameIntro.addLabel({ text: this.human === Player.defender ? 'defender' : 'attacker' });
            gameIntro.addLabel({
                text: this.hmlGameLogic.getUniversalWinner() === Player.defender ? 'Defender' : 'Attacker'
            });
            ...
            if (isGameOver) {
                ...
                if (currentPlayer === this.computer) {
                    this.hmlGameLogic.AutoPlay(this.computer, (updateTransitions) => {
                        updateTransitions.forEach((t) => this.processExplorer.drawProcess(t.targetProcess));
                        this.processExplorer.exploreProcess(
                            updateTransitions[updateTransitions.length - 1].targetProcess
                        );
                        this.processExplorer.focusOnProcess(
                            updateTransitions[updateTransitions.length - 1].targetProcess
                        );
                    });
                    this.refresh();
        ...
        private printGameOver(winner: Player, winReason: WinReason): void {
            ...
            switch (winReason) {
                ...
                case WinReason.stuck: {
                    ...
                    gameLogObject.addLabel({
                        text:
                            this.human === winner
                                ? this.computer === Player.defender
                                    ? 'Defender'
                                    : 'Attacker'
        ...
        private printCurrentConfig(process: CCS.Process, formula: HML.Formula, isNewRound = true): void {
            ...
            if (isNewRound) gameLogObject.setNewRound(true);

            gameLogObject.setTemplate('Current configuration: ({0}, {1}).');
            gameLogObject.addLabel({
                text: gameLogObject.labelForProcess(process),
                tag: '<span>',
                attr: [{ name: 'class', value: 'ccs-tooltip-process' }]
            });
            gameLogObject.addLabel({
                text: gameLogObject.labelForFormula(formula),
                tag: '<span>',
        ...
        private prepareGuiForUserAction() {
            ...
            if (this.hmlGameLogic.getNextActionType() === ActionType.transition) {
                ...
                this.transitionTable.setTransitions(
                    this.hmlGameLogic.state.process,
                    this.hmlGameLogic.getAvailableTransitions(),
                    this.hmlGameLogic.getCurrentSucc()
                );
    ...
    class HmlGameState {
        ...
        toString() {
            ...
            var processStr =
                this.process instanceof CCS.NamedProcess
                    ? (<CCS.NamedProcess>this.process).name
                    : this.process.id.toString();
    ...
    class HmlGameLogic {
        ...
        constructor(
            process: CCS.Process,
            formula: HML.Formula,
            formulaSet: HML.FormulaSet,
            strongSuccGen: CCS.SuccessorGenerator,
            weakSuccGen: CCS.SuccessorGenerator,
            graph: CCS.Graph
        ) {
            this.cycleCache = {};
            this.previousStates = [];
        ...
        public selectedFormula(formula: HML.Formula): void {
            ...
            gameLogPlay.addLabel({
                text:
                    this.getCurrentPlayer() === this.human
                        ? 'You ' + (this.human === Player.defender ? '(defender)' : '(attacker)')
                        : this.computer === Player.defender
                          ? 'Defender'
                          : 'Attacker'
            ...
            gameLogPlay.addLabel({
                text: gameLogPlay.labelForFormula(formula),
                tag: '<span>',
                attr: [{ name: 'class', value: 'monospace' }]
            });
        ...
        public selectedTransition(transition: CCS.Transition, usedTransitions: Function): void {
            ...
            gameLogPlay.addLabel({
                text: gameLogPlay.labelForProcess(this.state.process),
                tag: '<span>',
                attr: [{ name: 'class', value: 'ccs-tooltip-process' }]
            });
                    ...
                    attr: [
                        { name: 'class', value: 'ccs-tooltip-data' },
                        {
                            ...
                            value: Tooltip.strongSequence(
                                <Traverse.AbstractingSuccessorGenerator>this.weakSuccGen,
                                this.state.process,
                                transition.action,
                                transition.targetProcess,
                                this.graph
                            )
                        }
                    ]
            ...
            gameLogPlay.addLabel({
                text: gameLogPlay.labelForProcess(transition.targetProcess),
                tag: '<span>',
                attr: [{ name: 'class', value: 'ccs-tooltip-process' }]
            });
            this.writeToGamelog(gameLogPlay);
            ...
            if (wasWeak) {
                strictPath = (<any>this.weakSuccGen).getStrictPath(
                    fromProc.id,
                    transition.action,
                    transition.targetProcess.id
                );
            }
        ...
        public isGameOver(): Pair<Player, WinReason> {
            ...
            if (
                (!availTranstition || availTranstition.length <= 0) &&
                this.getNextActionType() === ActionType.transition
            ) {
        ...
        public getNextActionType(): ActionType {
            ...
            var transitionMoves = [
                HML.StrongForAllFormula,
                HML.WeakForAllFormula,
                HML.StrongExistsFormula,
                HML.WeakExistsFormula
            ];
        ...
        public JudgeUnfold(hml: HML.Formula, hmlFSet: HML.FormulaSet): HML.Formula {
            if (hml instanceof HML.MinFixedPointFormula) {
                ...
                if (namedFormula) {
                    if (
                        namedFormula instanceof HML.MinFixedPointFormula ||
                        namedFormula instanceof HML.MaxFixedPointFormula
                    ) {
                        var isMinVariable = namedFormula instanceof HML.MinFixedPointFormula;
                        var unfolded = this.JudgeUnfold(namedFormula, hmlFSet);
                        /* Gamelog */
                        var gameLogPlay = new GUI.Widget.GameLogObject(this.graph);
                        gameLogPlay.setTemplate(
                            'The variable {0} has been unfolded to {1}.<br>As {2} is defined as {3} fixed point,<br>a detected loop will become a win for {4}.'
                        );
                        gameLogPlay.addWrapper({ tag: '<p>' });
                        gameLogPlay.addLabel({
                            text: gameLogPlay.labelForFormula(hml),
                            tag: '<span>',
                            attr: [{ name: 'class', value: 'monospace' }]
                        });
                        gameLogPlay.addLabel({
                            text: gameLogPlay.labelForFormula(unfolded),
                            tag: '<span>',
                            attr: [{ name: 'class', value: 'monospace' }]
                        });
                        gameLogPlay.addLabel({
        ...
        public getAvailableTransitions(): CCS.Transition[] {
            if (this.getNextActionType() === ActionType.transition) {
                ...
                var availableTransitions = allTransitions.filter((transition) =>
                    hml.actionMatcher.matches(transition.action)
                );
