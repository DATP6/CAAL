...
messageHandlers.isStronglyBisimilar = (data) => {
    var attackSuccGen = CCS.getSuccGenerator(graph, {
            inputMode: inputMode,
            time: data.time,
            succGen: 'strong',
            reduce: true
...
messageHandlers.isWeaklyBisimilar = (data) => {
    var attackSuccGen = CCS.getSuccGenerator(graph, {
        ...
        defendSuccGen = CCS.getSuccGenerator(graph, {
            inputMode: inputMode,
            time: data.time,
            succGen: 'weak',
            reduce: true
...
messageHandlers.isStronglyTraceEq = (data) => {
    ...
    var leftToRightTraceInclusion = Equivalence.isTraceIncluded(
        attackSuccGen,
        defendSuccGen,
        leftProcess.id,
        rightProcess.id,
    ...
    if (!leftToRightTraceInclusion.isSatisfied) {
        ...
        rightToLeftTraceInclusion = Equivalence.isTraceIncluded(
            attackSuccGen,
            defendSuccGen,
            rightProcess.id,
            leftProcess.id,
...
messageHandlers.checkFormula = (data) => {
    var strongSuccGen = CCS.getSuccGenerator(graph, { inputMode: inputMode, succGen: 'strong', reduce: true }),
        ...
        result = DependencyGraph.solveMuCalculus(
            formulaSet,
            formula,
            strongSuccGen,
            weakSuccGen,
            graph.processByName(data.processName).id
        );
    data.result = result;
    self.postMessage(data);
};
...
messageHandlers.findDistinguishingFormula = (data) => {
    var strongSuccGen = CCS.getSuccGenerator(graph, {
            inputMode: inputMode,
            time: data.time,
            succGen: 'strong',
            reduce: true
        ...
        weakSuccGen =
            data.succGenType === 'weak'
                ? CCS.getSuccGenerator(graph, { inputMode: inputMode, time: data.time, succGen: 'weak', reduce: true })
                : strongSuccGen,
