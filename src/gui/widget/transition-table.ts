...
module GUI.Widget {
    ...
    export class TransitionTable {
        ...
        constructor() {
            ...
            $table.addClass(
                'widget-transition-table table table-responsive table-striped table-condensed table-hover no-highlight'
            );
            $table.append(
                '<thead><tr><th class="narrow">Source</th><th class="narrow">Action</th><th class="narrow">Target</th></tr></thead>'
            );
        ...
        setTransitions(source, transitions: CCS.Transition[], abstractingSuccGen: CCS.SuccessorGenerator) {
            ...
            this.transitions.forEach((transition, index) => {
                ...
                if (abstractingSuccGen instanceof Traverse.AbstractingSuccessorGenerator) {
                    ...
                    $action = $('<td></td>').append(
                        Activity.Tooltip.setTooltip(
                            Activity.Tooltip.wrap(actionTransition),
                            Activity.Tooltip.strongSequence(
                                abstractingSuccGen,
                                source,
                                transition.action,
