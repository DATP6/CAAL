var examples: any[] = [
        ...
        properties: [
            ...
            },
            {
                ...
                options: {
                    process: 'Peterson',
                    definitions:
                        'MutualExclusion max= [[enter1]][[enter2]]ff and [[enter2]][[enter1]]ff and [-]MutualExclusion;',
                    topFormula: 'MutualExclusion;',
                    comment:
                        'An invariant property that guarantees that immediately after enter1 it is not possible to perform enter2 and vice versa. '
                }
            }
        ],
        inputMode: 'CCS'
    ...
    {
        title: 'Small Prop Book Example',
        ccs: [
            "One = 'in.(A 0.8 B);",
            "A = out.One;",
            "B = err.One;",
            '',
            "Two = 'in.(C 0.8 (D 0.1 E));",
            "C = out.Two;",
            "D = err.Two;",
            "E = err.Two;",
        ].join('\n'),
        ...
        properties: [
            {
                ...
                options: {
                    ...
                    secondProcess: 'Spec',
                    comment:
                        'Trace inclusion guarantees that the system must after every enter perform (possibly after some tau and try sequence) the exit action.'
                }
            },
            {
                className: 'HML',
                ...
                options: {
                    process: 'System',
                    definitions:
                        'Safe max= [[enter]]NoMoreEnter and [-]Safe;\nNoMoreEnter max= [[enter]]ff and [[try]]NoMoreEnter;',
                    topFormula: 'Safe; ',
                    comment:
                        'Describes that it is impossible at any moment to perform two enters after each other, with only tau and try actions in between (after each enter we must eventually see exit before enter can be done again).'
                }
            },
            {
                ...
                options: {
                    ...
                    secondProcess: 'Spec',
                    comment:
                        'Stronger property than trace inclusion but because it holds, it implies also trace inclusion and verification of simulation usually takes less time than trace inclusion.'
                }
            }
        ],
        inputMode: 'TCCS'
        ...
        properties: [
            {
                ...
                options: {
                    ...
                    comment:
                        'The protocol guarantees mutual exclusion as the implementation and specification are weakly bisimilar.'
                }
            },
            {
                className: 'HML',
                ...
                options: {
                    ...
                    comment:
                        'The fixed point expresses the invariant that it is never possible to perform twice the action enter critical section after each other.'
                }
            }
        ],
        inputMode: 'CCS'
