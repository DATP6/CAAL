...
class Renderer {
    ...
    public redraw() {
        ...
        this.particleSystem.eachEdge((edge: Edge, pt1: Point, pt2: Point) => {
            ...
            if (edge.data.datas[0]?.dashed) {
                // if the edge is a probabilistic edge, draw a dashed line.
            ...
            if (isSelfloop) {
                this.drawSelfEdge(
                    ...
                    chevronColor,
                    label,
                    this.nodeBoxes[edge.target.name]
                );
                ...
                if (edge.source == oppo.target && edge.target == oppo.source) {
                    this.drawBendingEdge(
                        pt1,
                        pt2,
                        this.nodeBoxes[edge.source.name],
                        this.nodeBoxes[edge.target.name],
                        arrowLength,
                        arrowWidth,
                        chevronColor,
    ...
    private intersect_line_box(p1: Point, p2: Point, boxTuple: any): Point {
        ...
        return (
            this.intersect_line_line(p1, p2, tl, tr) ||
